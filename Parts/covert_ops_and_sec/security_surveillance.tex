...
\textit{Electrolaser Fence:}\label{itm:electrolaser_fence} An electrical fence using energy beams instead of wires. It delivers a HT-6 (2) affliction attack plus linked 1d-3 burn damage; use the rules for military electrolasers (p. \pageref{sec:electrolasers}). The fence can be set to ``stun'' or ``kill.'' \$5,000 per post for an open fence, double cost for a tight fence. LC3.
...
Electronic locks may also incorporate a scanlock (p. \pageref{itm:briometric_scanlock}) for additional security. 
...
This microbot swarm (p. \pageref{subsubsec:microbot}) is equipped with emissions sensors. Each square yard of bughunters can sweep one square yard per minute to locate nanobugs, comm taps, surevillance swarms, or other surevillance devices. They have Electronics Operation (Surveillance)-12 for this function only, or give +2 (quality) to an operator's skill. \$4,000/square yard. LC3.
