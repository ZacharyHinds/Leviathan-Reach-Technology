...
Many battlesuits are used with flight systems, since the suit's strength amplification makes it easy to carry extra gear. For maximum flexibility, these systems are not included in the suit designs, but are usually worn as extra gear. For maximum flexibility, these systems are not included in the suit designs, but are usually worn as external packs or belts. See \textit{Flight Pack} (p. TODO) for various options. Flight systems interface with battlesuits, allowing the suit's own navigation displays to be used for flight control.

\subsubsection{Underwater Battlesuits}\label{subsubsec:underwater_battlesuits}
Units operating underwater will need to use an aquatic propulsion pack (p. TODO) for submerged mobility. A few suits have integral aquatic propulsion.
...
Different variations of combat walkers are described below. These battlesuits have mostly become obsolete, having been replaced by heavy battlesuits (p. TODO), but vintage and refurbished walkers can still be found.
...
Powered combat armor gives +10 to Lifting and Striking ST and Super Jump 1. Biomedical Sensors (p. TODO) and a waste relief system (p. TODO) are standard features. The suit's surface has a tactical ESM (p. \pageref{itm:tactical_esm_detector}).
...
The suit's helmet includes a filter mask (p. \pageref{subsubsec:filter_mask}), an inertial compass (p. \pageref{itm:inertial_compass}), hearing protection (p. \pageref{subsec:threat_protection}), a hyperspectral visor (p. \pageref{itm:hyperspec_goggles}), a small laser comm (p. \pageref{itm:laser_comm_small}), and a medium radio (p. \pageref{itm:radio_comm_medium}). The suit's body incorporates biomedical sensors (p. TODO), trauma maintenance (p. TODO), a provisions dispenser (p. TODO) with a week's provisions, tactical ESM (p. \pageref{itm:tactical_esm_detector}), and a waste relief system (p. TODO). The suit also has infrared cloaking (p. \pageref{subsec:infrared_cloaking}) and radar stealth (p. \pageref{subsec:radar_stealth}).
