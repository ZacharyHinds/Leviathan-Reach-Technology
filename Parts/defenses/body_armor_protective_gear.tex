...
\subsection{Armor Features}\label{subsec:armor_features}
Modern armor without microclimate systems (p.\pageref{subsec:threat_protection}) incorporate undergarments with moisture-wicking (\textbf{GURPS High-Tech}, p. 64) properties. Any armor that covers the torso includes attachment points that make it the equivalent of a basic-quality load-bearing vest (\textbf{High-Tech}, p. 54). Rigid armor vests provide attachment points for backpacks (\textbf{High-Tech}, p. 54) and climbing equipment (\textbf{High-Tech}, p. 55). Arm and leg armor have pockets for the insertion of knee or elbow pads (\textbf{High-Tech}, p. 71).
...
\textit{Suit:}\label{itm:suit_body_armor} This head~-to~-toe outfit includes a hood and face mask with eye slit. It is used as the basis for tailored armor (p. \pageref{sec:tailoring_armor}).
...
Additional styles can be created using the tailored armor (p. \pageref{sec:tailoring_armor}) rules.
...
These are used when full~-equipped suit is unavailable or inappropriate. Each mask covers the entire face, providing the Protect Vision and Protect Smell advantages. For as long as the oxygen supply holds out, they effectively provide the Doesn't Breathe advantage. 
...
\subsubsection{Liquid Breathing Rig}\label{subsubsec:liquid_breathing_rig}
A liquid breathing rig consists of an assisted breathing system (with mask) and a tank of sterile oxygenated liquid. A recycling system treats used liquid and recharges it with oxygen from a separate air tank (p. \pageref{subsubsec:air_tanks}). (Artificial gills cannot be used at extreme depths because there is not enough usable oxygen to extract.) 

The system provides Pressure Support 2 and eliminates the usual problems associated with breathing high pressure gases, allowing arbitrarily fast descents and ascents without decompression. However, breathing the liquid is tiring -- even light exertion costs 1 FP per hour -- and speaking is impossible. Usually worn with a drysuit. \$20,000, 20 lb.
...
Tacsuits incorporate biomedical sensors (p. \pageref{subsec:biomedical_sensors}). With an air mask (p. \pageref{subsubsec:air_mask}) or combat infantry helmet (p. \pageref{subsubsec:combat_infantry_helmet}), the suit is sealed and provdes climate control (-40\degree{}F to 120\degree{}F).
...
It incorporates an inner garment including biomedical sensors (p. \pageref{subsec:biomedical_sensors}), a waste relief system (p. \pageref{itm:waste-relief_system}), and a microclimate control system (p. \pageref{subsec:threat_protection}). The back of the torso clamshells open so the user can step into the armor (it takes three seconds to step in or out). The helmet is \textit{not} included. When worn with either a combat infantry helmet (p. \pageref{subsubsec:combat_infantry_helmet}) or space helmet (p. \pageref{subsubsec:space_helmet}) the suit is sealed, with climate control (-140\degree{}F to 140\degree{}F) and radiation protection (PF 5).
